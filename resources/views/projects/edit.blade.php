            ...
            <div class="card-body">
                <form action="{{ route('projects.edit', ['id' => $project->id]) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="project_date" id="project_date" class="form-control" value="{{ old('project_date', $project->project_date) }}">
                    </div>

                    <div class="form-group">
                        <label for="id_label_multiple">Người thực hiện <span class="text-danger">*</span></label>
                        <select name="user_id[]" class="form-control" id="id_label_multiple" multiple required>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ (collect(old('user_id', $project->users->pluck('id')))->contains($user->id)) ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
